...
We provide a Hydra config for inference at [configs/inference/default.yaml](configs/inference/default.yaml). Edit that file for persistent experiment defaults, or override individual values from the command line. Run `python -u inference.py --cfg job` to print the resolved inference config without starting inference. Pretrained weights will be **automatically downloaded**. For users with limited VRAM, please run the following scripts with [tiled sampling](#tiled-sampling).
...
upscale=4 \
model.version=v2 \
sampling.sampler=spaced \
sampling.steps=50 \
caption.name=none \
caption.pos_prompt='' \
caption.neg_prompt='low quality, blurry, low-resolution, noisy, unsharp, weird textures' \
sampling.cfg_scale=4 \
io.input=inputs/demo/bsr \
io.output=results/v2_demo_bsr \
runtime.device=cuda runtime.precision=fp32
...
model.version=v2.1 \
caption.name=llava \
sampling.cfg_scale=8 \
sampling.noise_aug=0 \
io.input=inputs/demo/bsr \
io.output=results/v2.1_demo_bsr
```
...
io.input=inputs/demo/bfr/aligned \
io.output=results/v2_demo_bfr_aligned \
runtime.device=cuda runtime.precision=fp32
...
sampling.noise_aug=0 \
io.input=inputs/demo/bfr/aligned \
io.output=results/v2.1_demo_bfr_aligned
```
...
caption.neg_prompt='low quality, blurry, low-resolution, noisy, unsharp, weird textures' \
sampling.cfg_scale=4.0 \
io.input=inputs/demo/bfr/whole_img \
io.output=results/v2_demo_bfr_unaligned \
runtime.device=cuda runtime.precision=fp32
...
sampling.noise_aug=0 \
io.input=inputs/demo/bfr/whole_img \
io.output=results/v2.1_demo_bfr_unaligned
```
...
caption.neg_prompt='low quality, blurry, low-resolution, noisy, unsharp, weird textures' \
sampling.cfg_scale=4.0 \
io.input=inputs/demo/bid \
io.output=results/v2_demo_bid \
runtime.device=cuda runtime.precision=fp32
...
sampling.noise_aug=0 \
io.input=inputs/demo/bid \
io.output=results/v2.1_demo_bid
```
...
model.train_cfg=/path/to/training/config.yaml \
model.ckpt=/path/to/saved/checkpoint.pt \
caption.name=llava \
sampling.cfg_scale=8 \
sampling.noise_aug=0 \
io.input=inputs/demo/bsr \
io.output=results/custom_demo_bsr
```
...
tiling.cleaner.enabled=true \
tiling.cleaner.tile_size=256 \
tiling.cleaner.tile_stride=128 \
# tiled inference for VAE encoding
tiling.vae_encoder.enabled=true \
tiling.vae_encoder.tile_size=256 \
# tiled inference for VAE decoding
tiling.vae_decoder.enabled=true \
tiling.vae_decoder.tile_size=256 \
# tiled inference for diffusion process
tiling.cldm.enabled=true \
