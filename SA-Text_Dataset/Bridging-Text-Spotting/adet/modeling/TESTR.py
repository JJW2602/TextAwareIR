...
class TransformerDetector(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # save_image(images[0], './img0.jpg', normalize=True)  # BGR
        ...
        img0 = tmp_images.tensor[0]
        img0 = (img0-img0.min())/(img0.max()-img0.min()) * 255.0
        img0 = img0.permute(1,2,0).detach().cpu().numpy().astype(np.uint8)
        img0 = np.ascontiguousarray(img0)
        box0 = batched_inputs[0]['instances'].gt_boxes.tensor.detach().cpu().numpy().astype(np.uint8)
        poly0 = batched_inputs[0]['instances'].polygons.detach().cpu().numpy().astype(np.uint8)

        img1 = tmp_images.tensor[1]
        img1 = (img1-img1.min())/(img1.max()-img1.min()) * 255.0
        img1 = img1.permute(1,2,0).detach().cpu().numpy().astype(np.uint8)
        img1 = np.ascontiguousarray(img1)
        box1 = batched_inputs[1]['instances'].gt_boxes.tensor.detach().cpu().numpy().astype(np.uint8)
        poly1 = batched_inputs[1]['instances'].polygons.detach().cpu().numpy().astype(np.uint8)
        cv2.imwrite('./img0.jpg', img0[:,:,::-1])
        cv2.imwrite('./img1.jpg', img1[:,:,::-1])

        for box in box0:
            x1,y1,x2,y2 = box 
            cv2.rectangle(img0, (x1,y1), (x2,y2), (255,0,0), 4)
        cv2.imwrite(f'./img0_box.jpg', img0[:,:,::-1])

        for poly in poly0:
            poly = poly.reshape(-1,2).astype(np.int32)
            cv2.polylines(img0, [poly], isClosed=True, color=(0,255,0), thickness=4)
        cv2.imwrite(f'./img0_poly.jpg', img0[:,:,::-1])

        for poly in poly1:
            poly = poly.reshape(-1,2).astype(np.int32)
            cv2.polylines(img1, [poly], isClosed=True, color=(0,255,0), thickness=4)
        cv2.imwrite(f'./img1_poly.jpg', img1[:,:,::-1])

        for box in box1:
            x1,y1,x2,y2 = box 
            cv2.rectangle(img1, (x1,y1), (x2,y2), (255,0,0), 4)
        cv2.imwrite(f'./img1_box.jpg', img1[:,:,::-1])
        ...
        if self.training:
            ...
            img1 = tmp_images.tensor[0].clone()
            img1 = (img1-img1.min()) / (img1.max()-img1.min()) * 255.0
            img1 = img1.permute(1,2,0).cpu().numpy().astype(np.uint8)
            img1_vis = img1.copy()
            x,y,w,h = map(int,target_boxes[0][0].tensor.tolist()[0])
            cv2.circle(img1_vis, (x,y), 10, (255,0,0), -1)
            cv2.circle(img1_vis, (w,h), 10, (255,0,0), -1)
            x,y,w,h = map(int,target_boxes[0][1].tensor.tolist()[0])
            cv2.circle(img1_vis, (x,y), 10, (255,0,0), -1)
            cv2.circle(img1_vis, (w,h), 10, (255,0,0), -1)
            x,y,w,h = map(int,target_boxes[0][2].tensor.tolist()[0])
            cv2.circle(img1_vis, (x,y), 10, (255,0,0), -1)
            cv2.circle(img1_vis, (w,h), 10, (255,0,0), -1)
            cv2.imwrite(f'./img1_tmp.jpg', img1_vis[:,:,::-1])

            img1 = tmp_images.tensor[1].clone()
            img1 = (img1-img1.min()) / (img1.max()-img1.min()) * 255.0
            img1 = img1.permute(1,2,0).cpu().numpy().astype(np.uint8)
            img1_vis = img1.copy()
            x,y,w,h = map(int,target_boxes[1][0].tensor.tolist()[0])
            cv2.circle(img1_vis, (x,y), 10, (255,0,0), -1)
            cv2.circle(img1_vis, (w,h), 10, (255,0,0), -1)
            cv2.imwrite(f'./img2_tmp.jpg', img1_vis[:,:,::-1])
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            # breakpoint()
            h, w = targets_per_image.image_size     # height, width
